...
def uv_installed_pythons() -> list[uv.UVPythonListing]:
    pys = [
        uv.UVPythonListing.from_dict(v)
        for v in json.loads(uv_install_json())
    ]
    return pys
...
def test_check_uv():
    ...
    with patch("subprocess.run", side_effect=subprocess.CalledProcessError(1, "")) as fake_process:
        out = uv.check_uv()
        fake_process.assert_called()
        assert out is False
...
def test_uv_python_dir():
    with patch("subprocess.run") as fake_process:
        cmd_output = MagicMock()
        cmd_output.stdout = "C:\\Users\\ducks\\AppData\\Roaming\\uv\\python\n"
        fake_process.return_value = cmd_output

        out = uv.uv_python_dir()
        fake_process.assert_called()
        assert out == "C:\\Users\\ducks\\AppData\\Roaming\\uv\\python"
...
def test_find_matching_listing():
...
def test_install():
    ...
    with patch("subprocess.run") as fake_process:
        ...
        fake_process.assert_called_once_with(
            ["uv", "python", "install", listing.key, "--color", "never", "--no-progress"],
            capture_output=True,
            text=True,
            check=True,
        )
...
def test_uninstall():
    listing = UVPythonListing(
        ...
        path='C:\\Users\\ducks\\AppData\\Roaming\\uv\\python\\cpython-3.13.2-windows-x86_64-none\\python.exe',
        symlink=None,
        url=None,
        os='windows',
        variant='default',
    ...
    with patch("subprocess.run") as fake_process:
        ...
        fake_process.assert_called_once_with(
            ["uv", "python", "uninstall", listing.key, "--color", "never", "--no-progress"],
            capture_output=True,
            text=True,
            check=True,
        )
